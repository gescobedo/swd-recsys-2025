...
class GSW(torch.nn.Module):
    ...
    def gswt(self, X, Y, theta=None):
        """
        Calculates Truncated-GSW between two empirical distributions.
        Note that the number of samples is assumed to be equal
        (This is however not necessary and could be easily extended
        for empirical distributions with different number of samples)
        """
        ...
        return torch.sqrt(
            torch.sum(
                (Xslices_sorted[:truncate_point] - Yslices_sorted[:truncate_point]) ** 2
            )
        )

    def forward(self, X: torch.Tensor, Y: torch.Tensor, theta=None):
        if self.method == "gsw":
            return self.gsw(X, Y)
        elif self.method == "gswt":
            return self.gswt(X, Y)
        elif self.method == "max-gsw":
            return self.max_gsw(X, Y)
        else:
            raise NotImplementedError("Distance method not defined")
    ...
    def max_gsw(self, X, Y, iterations=50, lr=1e-4):
        ...
        for i in range(iterations):
            optimizer.zero_grad()
            loss = -self.gsw(
                X.to(self.device), Y.to(self.device), self.theta.to(self.device)
            )
