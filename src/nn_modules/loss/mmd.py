...
class MMD(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        loss = self.mmd_loss(
            embeddings, labels, kernel_mul=100, kernel_num=4, fix_sigma=True
        )
        return loss
    ...
    def gaussian_kernel(self, src_embed, tgt_embed, kernel_mul, kernel_num, fix_sigma):
        ...
        kernel_val = [
            torch.exp(-l2_distance / (bandwidth_temp + 1e-5))
    ...
    def mmd(self, src_embed, tgt_embed, kernel_mul, kernel_num, fix_sigma):
        ...
        # Upsammpling to the bigger group
        batch_size = (
            src_batch_size if src_batch_size > tgt_batch_size else tgt_batch_size
        )
        ...
        kernels = self.gaussian_kernel(
            src_embed_rep, tgt_embed_rep, kernel_mul, kernel_num, fix_sigma
        )
