...
ACTIVATION_FN_MAP = {"relu": nn.ReLU(), "tanh": nn.Tanh(), "sigmoid": nn.Sigmoid()}
...
class PolyLinear(nn.Module):
    def __init__(
        ...
        assert len(layer_config) > 1, (
            "For a linear network, we at least need one "
            "input and one output dimension"
        )

        self.layer_config = layer_config
        activation_fn = (
            ACTIVATION_FN_MAP[activation_fn]
        ...
        for i, (d1, d2) in enumerate(zip(layer_config[:-1], layer_config[1:])):
            ...
            if i < self.n_layers - 1:
                ...
                layer_dict[f"{activation_fn.__class__.__name__.lower()}_{i}"] = (
                    activation_fn
                )
