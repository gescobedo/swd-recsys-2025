...
class NegativeSampler(InteractionSampler):
    ...
    def __init__(
        ...
        squashing_factor_pop_sampling: float = 1.0,
    ):
        """
        ...
        assert neg_sampling_strategy in [
            "uniform",
            "popular",
        ], f"<{neg_sampling_strategy}> is not a valid negative sampling strategy!"
        assert (
            squashing_factor_pop_sampling >= 0
        ), "Squashing factor for popularity sampling should be positive!"
    ...
    def _neg_sample_popular(self, to_sample: int):
        ...
        p = np.power(
            p, self.squashing_factor_pop_sampling
        )  # Applying squashing factor alpha
        p = p / p.sum()
        return np.random.choice(np.arange(self.n_items), size=to_sample, p=p)


class NegSamplingDataLoader(DataLoader):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.interaction_sampler = NegativeSampler(
            train_dataset=dataset,
            n_neg=n_neg_samples,
    ...
    def sampling(self, n_items):
        return np.random.choice(self.dataset.items_in_split, size=n_items, replace=True)
    ...
    def _neg_sampling_collate_fn(self, batch):
        ...
        # user_feat = [x[-1][0] for x in batch]
        # item_feat = [x[-1][1] for x in batch]
        ...
        #         mask[i] = np.isin(
        #             item_neg_idxs[i],
        #             self.interaction_sampler.dataset.sampling_matrix[
        #                 user_idxs[i]
        #             ].indices,
        #             assume_unique=True,
        #         )
        ...
        n_positive = (
            item_pos_indices.shape[-1] if len(item_pos_indices.shape) > 1 else 1
        )
        positive_user_items = [
            self.dataset.sampling_row_indices[idx] for idx in user_indices
        ]
        ...
        user_feat = np.array([x[-1][0] for x in batch])
        item_feat = np.array([x[-1][1] for x in batch])
        # labels = np.zeros_like(items_idxs, dtype=float)
        # labels[:, :n_pos] = 1.0
        return (
            torch.from_numpy(user_indices),
            torch.from_numpy(item_pos_indices),
            torch.from_numpy(item_neg_indices).flatten(),
            [],
            (torch.from_numpy(user_feat), torch.from_numpy(item_feat)),
        )
