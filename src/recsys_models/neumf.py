...
class NeuMF(BaseRecModel):
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Embedding):
            general_weight_init(module.weight.data, mean=0.0, std=0.01)
    ...
    def build_from_config(self, config: dict, dataset: BaseDataset):
        config.update({"n_items": dataset.n_items, "n_users": dataset.n_users})
        raise NeuMF(config, dataset)
